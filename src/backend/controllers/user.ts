...
import { userService } from '../services/user';
 ...
 const delete_user = async (
  ...
  try {
    ...
    let prams_id = params.id
    let id = parseInt(prams_id)
    user = await userService.delete_my_user(id)
 ...
 const update_user_password = async (
  ...
  try {
    ...
    const validatedBody = userValidator.validator_user_update_password_body(body)

    const updatedUser = await userService.update_my_user_password(validatedBody, accessToken)
